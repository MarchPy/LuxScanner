...
class FundamentusScraper:
    ...
    def fundamentus(self, tickers: list):
        ...
        for ticker in tickers:
            ...
            try:
                ...
                self.console.print(f'[[bold white]{self.time()}[/]] -> [[bold green]Dados coletados para o ativo[/]] :: {ticker}')
...
class Main:
    ...
    def main(self, save_file=False):
        ...
        df_fundamentus = self.scraper.fundamentus(tickers=tickers)
        
        if save_file:
            df_fundamentus.to_excel(f'Relatório de indicadores fundamentalistas ({self.scraper.date()}).xlsx')
